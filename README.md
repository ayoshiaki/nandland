...
|:---:|--------------------------------------|--------------------------------------------------|----------------------|
| 1  | Pinos e simulador combinacional     | `pinos.py`, `componentes.py`, `simulador.py`     | 1.1–1.3, 2.1–2.5     |
| 2  | Porta NAND + circuitos compostos    | `circuito.py`, `biblioteca.py` (parcial)         | 3.1–3.4 (+ `construir_not`) |
| 3  | Lógica sequencial (latches, FF)     | `biblioteca.py` (sequencial)                     | 4.1–4.4              |
| 4  | *(opcional)* GUI                    | extra                                            | Fase 5               |
...
<details>
<summary><strong>4. Rode os testes antes de abrir o Pull Request</strong></summary>

```bash
python -m pytest -q
```

Informe no corpo do Pull Request quais testes passam — ex.: `Fase 1: 12/12`, `Fase 2: 8/8`.
</details>
...
<details>
<summary><strong>6. Descrição do Pull Request</strong></summary>

Deve conter:
- O que foi implementado (passos cobertos, ex.: 2.1–2.5).
- Como rodar/validar (comando de testes + resultado esperado).
...
Responda aos comentários com novos commits na mesma branch — o Pull Request atualiza sozinho. Evite `force-push` após o início da revisão, para preservar o histórico da discussão.
</details>
